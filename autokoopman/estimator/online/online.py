...
class OnlineInputDMD:
    ...
    """
    def __init__(self, n: int, m: int, weighting = 0.9) -> None:
        self.n = n
        self.m = m

        self.weighting = weighting
        self.timestep = 0
        self.A = np.zeros((n, n+m))
        self._P = np.zeros((n+m, n+m))
        self._initialize()
        self.ready = False
    ...
    def initialize(self, Xp: np.ndarray, Yp: np.ndarray, Up: np.ndarray) -> None:
        assert Xp is not None and Yp is not None
        Xp, Yp, Up = np.array(Xp), np.array(Yp), np.array(Up)
    ...
    def update(self, x: np.ndarray, y: np.ndarray, u: np.ndarray) -> None:
        ...
        # update A
        self.A += np.outer(gamma * (y - self.A.dot(x)), Pz)
